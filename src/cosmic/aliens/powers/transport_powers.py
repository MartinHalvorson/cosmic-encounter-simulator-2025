"""
Transport Powers - Transportation themed aliens.
"""
...
@dataclass
class Ship_Transport(AlienPower):
    """Ship_Transport - Sea vessel. +4 on offense."""
    name: str = field(default="Ship_Transport", init=False)
...
class Train(AlienPower):
    """Train - Rail power. +3 first encounter."""
...
@dataclass
class Plane(AlienPower):
    """Plane - Air transport. See opponent card."""
    name: str = field(default="Plane", init=False)
...
@dataclass
class Rocket_Transport(AlienPower):
    """Rocket_Transport - Space travel. +5 offense, -2 defense."""
    name: str = field(default="Rocket_Transport", init=False)
...
class Submarine(AlienPower):
    """Submarine - Underwater. Ships go home."""
...
class Helicopter(AlienPower):
    """Helicopter - Vertical lift. +2 always."""
...
class Motorcycle(AlienPower):
    """Motorcycle - Fast. Retrieve 3 ships."""
...
@dataclass
class Tank_Transport(AlienPower):
    """Tank_Transport - Armed vehicle. +4 on defense."""
    name: str = field(default="Tank_Transport", init=False)
...
class Bicycle(AlienPower):
    """Bicycle - Simple transport. +1 per card."""
...
class Ambulance(AlienPower):
    """Ambulance - Emergency. +3 defending home."""
...
class Yacht(AlienPower):
    """Yacht - Luxury vessel. -2 to opponent."""
...
@dataclass
class Cargo(AlienPower):
    """Cargo - Heavy hauler. +2 per home colony."""
    name: str = field(default="Cargo", init=False)
...
# Register all powers
TRANSPORT_POWERS = [
    Ship_Transport, Train, Plane, Rocket_Transport, Submarine,
    Helicopter, Motorcycle, Bus, Tank_Transport, Bicycle,
    Taxi, Ambulance, Yacht, Scooter, Cargo,
]

for power_class in TRANSPORT_POWERS:
    try:
