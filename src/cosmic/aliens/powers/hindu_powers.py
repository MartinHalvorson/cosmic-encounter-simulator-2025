"""
Hindu Powers - Hindu mythology themed aliens.
"""
...
class Ganesha(AlienPower):
    """Ganesha - Remover of Obstacles. +4 always."""
...
class Lakshmi(AlienPower):
    """Lakshmi - Fortune Goddess. Extra card draw."""
...
class Kali(AlienPower):
    """Kali - Dark Goddess. -3 to opponent."""
...
class Durga(AlienPower):
    """Durga - Warrior Goddess. +5 on offense."""
...
class Yama(AlienPower):
    """Yama - Death God. Opponent loses 1 ship."""
...
@dataclass
class Garuda(AlienPower):
    """Garuda - Divine Eagle. +3 always."""
    name: str = field(default="Garuda", init=False)
...
@dataclass
class Naga(AlienPower):
    """Naga - Serpent Being. +4 on defense."""
    name: str = field(default="Naga", init=False)
    description: str = field(default="+4 when defending.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
# Register all powers
HINDU_POWERS = [
    Brahma, Vishnu, Shiva, Ganesha, Hanuman, Lakshmi, Saraswati, Kali, Durga,
    Indra, Agni, Varuna, Yama, Garuda, Naga,
]

for power_class in HINDU_POWERS:
    try:
