"""
Tree Species Powers - Tree species themed aliens.
"""
...
class Sequoia(AlienPower):
    """Sequoia - Ancient Giant. +5 on defense."""
...
class Maple_Tree(AlienPower):
    """Maple_Tree - Syrup Maker. +4 always."""
...
class Birch_Tree(AlienPower):
    """Birch_Tree - White Bark. +3 always."""
...
class Cedar_Tree(AlienPower):
    """Cedar_Tree - Aromatic Wood. +4 on defense."""
...
class Pine_Tree(AlienPower):
    """Pine_Tree - Evergreen. +3 on defense."""
...
@dataclass
class Spruce(AlienPower):
    """Spruce - Conifer. +3 always."""
    name: str = field(default="Spruce", init=False)
    description: str = field(default="+3 constant.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
class Willow_Tree(AlienPower):
    """Willow_Tree - Weeping Tree. Ships escape warp."""
...
@dataclass
class Aspen(AlienPower):
    """Aspen - Quaking Tree. Random +2 to +5."""
    name: str = field(default="Aspen", init=False)
...
class Baobab(AlienPower):
    """Baobab - Water Tree. Return 3 ships from warp."""
...
class Banyan(AlienPower):
    """Banyan - Spreading Roots. +3 per ally."""
...
class Cypress(AlienPower):
    """Cypress - Swamp Tree. +4 on offense."""
...
@dataclass
class Juniper(AlienPower):
    """Juniper - Berry Tree. Draw extra card."""
    name: str = field(default="Juniper", init=False)
...
@dataclass
class Acacia(AlienPower):
    """Acacia - Savanna Tree. +4 always."""
    name: str = field(default="Acacia", init=False)
    description: str = field(default="+4 constant.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
# Register all powers
TREE_SPECIES_POWERS = [
    Redwood, Sequoia, Maple_Tree, Birch_Tree, Cedar_Tree, Pine_Tree, Spruce,
    Willow_Tree, Aspen, Baobab, Banyan, Cypress, Juniper, Magnolia, Acacia,
]

for power_class in TREE_SPECIES_POWERS:
    try:
