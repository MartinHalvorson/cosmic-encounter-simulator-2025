"""
Furniture Powers - Furniture themed aliens.
"""
...
class Couch(AlienPower):
    """Couch - Comfortable Seat. +2 per ally."""
...
class Desk(AlienPower):
    """Desk - Work Surface. +4 on offense."""
...
class Bookshelf(AlienPower):
    """Bookshelf - Knowledge Storage. Extra card draw."""
...
@dataclass
class Wardrobe(AlienPower):
    """Wardrobe - Clothes Storage. +4 on defense."""
    name: str = field(default="Wardrobe", init=False)
    description: str = field(default="+4 when defending.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
class Lamp(AlienPower):
    """Lamp - Light Source. See opponent card."""
...
class Throne(AlienPower):
    """Throne - Royal Seat. +5 on defense."""
...
class Mirror_Furniture(AlienPower):
    """Mirror_Furniture - Reflective Surface. Random +1 to +5."""
...
@dataclass
class Dresser(AlienPower):
    """Dresser - Drawer Unit. +4 always."""
    name: str = field(default="Dresser", init=False)
    description: str = field(default="+4 constant.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
@dataclass
class Rug(AlienPower):
    """Rug - Floor Cover. +3 on defense."""
    name: str = field(default="Rug", init=False)
...
class Chandelier(AlienPower):
    """Chandelier - Elegant Light. +5 always."""
...
class Hammock(AlienPower):
    """Hammock - Swing Bed. Ships escape."""
...
# Register all powers
FURNITURE_POWERS = [
    Chair, Table, Bed, Couch, Desk, Bookshelf, Wardrobe, Lamp, Cabinet,
    Throne, Mirror_Furniture, Dresser, Rug, Chandelier, Hammock,
]

for power_class in FURNITURE_POWERS:
    try:
