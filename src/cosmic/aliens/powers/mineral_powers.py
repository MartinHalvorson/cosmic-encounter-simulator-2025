"""
Mineral Powers - Rock and mineral themed aliens.
"""
...
@dataclass
class Granite(AlienPower):
    """Granite - Solid rock. +3 on defense."""
    name: str = field(default="Granite", init=False)
...
@dataclass
class Marble(AlienPower):
    """Marble - Beautiful stone. +2 always."""
    name: str = field(default="Marble", init=False)
...
class Quartz(AlienPower):
    """Quartz - Crystal clarity. See opponent card."""
...
class Obsidian(AlienPower):
    """Obsidian - Volcanic glass. +4 on offense."""
...
@dataclass
class Slate(AlienPower):
    """Slate - Layered rock. +1 per home colony."""
    name: str = field(default="Slate", init=False)
...
@dataclass
class Limestone(AlienPower):
    """Limestone - Sedimentary. Ships return home."""
    name: str = field(default="Limestone", init=False)
...
@dataclass
class Basite(AlienPower):
    """Basalt - Volcanic rock. +3 first encounter."""
    name: str = field(default="Basalt", init=False)
...
@dataclass
class Sandite(AlienPower):
    """Sandstone - Grainy rock. Random +1 to +4."""
    name: str = field(default="Sandstone", init=False)
    description: str = field(default="Random +1 to +4.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    ...
    def modify_total(self, game: "Game", player: "Player", total: int, side: Side) -> int:
        if player.power_active:
            return total + random.randint(1, 4)
        return total


@dataclass
class Chalk(AlienPower):
    """Chalk - Soft stone. Draw on win."""
...
@dataclass
class Shite(AlienPower):
    """Shale - Fragile layers. -2 to opponent."""
    name: str = field(default="Shale", init=False)
...
class Pumice(AlienPower):
    """Pumice - Light rock. Retrieve 2 ships."""
...
class Jade(AlienPower):
    """Jade - Precious stone. +1 per ally."""
...
@dataclass
class Flite(AlienPower):
    """Flint - Fire starter. +5 on offense, -2 defense."""
    name: str = field(default="Flint", init=False)
...
class Geode(AlienPower):
    """Geode - Hidden crystals. +1 per card in hand."""
...
# Register all powers
MINERAL_POWERS = [
    Granite, Marble, Quartz, Obsidian, Slate, Limestone, Basite,
    Sandite, Chalk, Shite, Pumice, Jade, Onyx, Flite, Geode,
]

for power_class in MINERAL_POWERS:
    try:
