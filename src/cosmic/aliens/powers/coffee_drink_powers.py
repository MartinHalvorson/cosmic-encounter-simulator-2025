"""
Coffee drink themed alien powers.
"""
...
class Espresso(AlienPower):
    """Espresso - Power of Intensity. Concentrated strength."""
...
class Latte(AlienPower):
    """Latte - Power of Smoothness. Balanced approach."""
...
class Cappuccino(AlienPower):
    """Cappuccino - Power of Layers. Complex strategy."""
...
class Mocha(AlienPower):
    """Mocha - Power of Sweetness. Rich combination."""
...
class Americano(AlienPower):
    """Americano - Power of Dilution. Spread influence."""
...
class Macchiato(AlienPower):
    """Macchiato - Power of Marks. Leave your mark."""
...
class Cold_Brew(AlienPower):
    """Cold_Brew - Power of Patience. Slow and steady."""
...
@dataclass
class Flat_White(AlienPower):
    """Flat_White - Power of Balance. Smooth operator."""
    name: str = field(default="Flat_White", init=False)
...
class Ristretto(AlienPower):
    """Ristretto - Power of Concentration. Pure essence."""
    name: str = field(default="Ristretto", init=False)
    description: str = field(default="+6 with 2 or fewer cards.", init=False)
    timing: PowerTiming = field(default=PowerTiming.RESOLUTION, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.YELLOW, init=False)
...
class Frappe(AlienPower):
    """Frappe - Power of Ice. Cool under pressure."""
...
class Cortado(AlienPower):
    """Cortado - Power of Cut. Direct approach."""
...
class Affogato(AlienPower):
    """Affogato - Power of Combination. Best of both."""
...
@dataclass
class Doppio(AlienPower):
    """Doppio - Power of Double. Twice the strength."""
    name: str = field(default="Doppio", init=False)
...
@dataclass
class Red_Eye(AlienPower):
    """Red_Eye - Power of Endurance. Push through."""
    name: str = field(default="Red_Eye", init=False)
...
@dataclass
class Vienna(AlienPower):
    """Vienna - Power of Elegance. Refined taste."""
    name: str = field(default="Vienna_Coffee", init=False)
    description: str = field(default="+3 plus +1 per ally (max +6).", init=False)
    timing: PowerTiming = field(default=PowerTiming.RESOLUTION, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
    ...
    def modify_total(self, game: "Game", player: "Player", total: int, side: Side) -> int:
        ...
        if side == Side.OFFENSE:
            ...
            ally_count = len([p for p in game.defense_allies if p != player.name])
        return total + min(6, 3 + ally_count)


COFFEE_DRINK_POWERS = [
    Espresso, Latte, Cappuccino, Mocha, Americano, Macchiato, Cold_Brew,
    Flat_White, Ristretto, Frappe, Cortado, Affogato, Doppio, Red_Eye, Vienna
]

for power_class in COFFEE_DRINK_POWERS:
    try:
