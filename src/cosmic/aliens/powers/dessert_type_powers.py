"""
Dessert type themed alien powers.
"""
...
@dataclass
class Ice_Cream(AlienPower):
    """Ice_Cream - Power of Cold. Frozen treat."""
    name: str = field(default="Ice_Cream", init=False)
...
@dataclass
class Cake(AlienPower):
    """Cake - Power of Celebration. Party time."""
...
@dataclass
class Pie(AlienPower):
    """Pie - Power of Slices. Divide and share."""
    name: str = field(default="Pie", init=False)
    description: str = field(default="+1 per ally (max +4).", init=False)
    timing: PowerTiming = field(default=PowerTiming.RESOLUTION, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
    ...
    def modify_total(self, game: "Game", player: "Player", total: int, side: Side) -> int:
        ...
        if side == Side.OFFENSE:
            ...
            ally_count = len([p for p in game.defense_allies if p != player.name])
        return total + min(4, ally_count)


@dataclass
class Cookie(AlienPower):
    """Cookie - Power of Crunch. Satisfying snack."""
    name: str = field(default="Cookie", init=False)
...
class Brownie(AlienPower):
    """Brownie - Power of Density. Rich and thick."""
...
class Cupcake(AlienPower):
    """Cupcake - Power of Portion. Single serving."""
...
@dataclass
class Pudding(AlienPower):
    """Pudding - Power of Softness. Gentle approach."""
    name: str = field(default="Pudding", init=False)
...
class Donut(AlienPower):
    """Donut - Power of Circles. Round and round."""
...
class Cheesecake(AlienPower):
    """Cheesecake - Power of Richness. Decadent indulgence."""
...
class Tiramisu(AlienPower):
    """Tiramisu - Power of Layers. Complex structure."""
...
@dataclass
class Mousse(AlienPower):
    """Mousse - Power of Air. Light and fluffy."""
    name: str = field(default="Mousse", init=False)
...
class Macaron(AlienPower):
    """Macaron - Power of Delicacy. Refined taste."""
...
@dataclass
class Tart(AlienPower):
    """Tart - Power of Sharpness. Tangy taste."""
    name: str = field(default="Tart", init=False)
    description: str = field(default="+4 when attacking.", init=False)
    timing: PowerTiming = field(default=PowerTiming.RESOLUTION, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
class Eclair(AlienPower):
    """Eclair - Power of Length. Elongated power."""
...
class Sorbet(AlienPower):
    """Sorbet - Power of Freshness. Clean palate."""
...
DESSERT_TYPE_POWERS = [
    Ice_Cream, Cake, Pie, Cookie, Brownie, Cupcake, Pudding, Donut,
    Cheesecake, Tiramisu, Mousse, Macaron, Tart, Eclair, Sorbet
]

for power_class in DESSERT_TYPE_POWERS:
    try:
