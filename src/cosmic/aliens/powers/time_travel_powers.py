"""
Time Travel Powers - Time travel themed aliens.
"""
...
@dataclass
class Chrononaut(AlienPower):
    """Chrononaut - Time Traveler. +5 always."""
    name: str = field(default="Chrononaut", init=False)
...
class Timeline(AlienPower):
    """Timeline - Alternate Path. See opponent card."""
...
class Rewind(AlienPower):
    """Rewind - Go Back. Return 1 ship from warp."""
...
class Fast_Forward(AlienPower):
    """Fast_Forward - Skip Ahead. +5 on offense."""
...
class Freeze_Frame(AlienPower):
    """Freeze_Frame - Stop Time. +5 on defense."""
...
class Temporal(AlienPower):
    """Temporal - Time Related. +4 always."""
...
@dataclass
class Era(AlienPower):
    """Era - Long Period. +4 on defense."""
    name: str = field(default="Era", init=False)
...
@dataclass
class Continuum(AlienPower):
    """Continuum - Endless Time. +4 always."""
    name: str = field(default="Continuum", init=False)
    description: str = field(default="+4 constant.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
@dataclass
class Flux(AlienPower):
    """Flux - Time Shift. Random +1 to +6."""
    name: str = field(default="Flux", init=False)
...
class Hourglass(AlienPower):
    """Hourglass - Time Counter. Extra card draw."""
...
class Clockwork(AlienPower):
    """Clockwork - Time Machine. +4 always."""
...
# Register all powers
TIME_TRAVEL_POWERS = [
    Chrononaut, Paradox, Timeline, Rewind, Fast_Forward, Freeze_Frame,
    Temporal, Epoch, Era, Eon, Moment, Continuum, Flux, Hourglass, Clockwork,
]

for power_class in TIME_TRAVEL_POWERS:
    try:
