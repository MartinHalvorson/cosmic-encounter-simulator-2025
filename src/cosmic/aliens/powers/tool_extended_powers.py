"""
Tool Extended Powers - More tool themed aliens.
"""
...
class Drill(AlienPower):
    """Drill - Boring Tool. +4 on offense."""
...
class Pliers(AlienPower):
    """Pliers - Gripping Tool. +4 on defense."""
...
class Screwdriver(AlienPower):
    """Screwdriver - Fastening Tool. +3 always."""
...
class Tape_Measure(AlienPower):
    """Tape_Measure - Measuring Tool. See opponent card."""
...
class Chisel(AlienPower):
    """Chisel - Carving Tool. +4 on offense."""
...
class File_Tool(AlienPower):
    """File_Tool - Smoothing Tool. +3 on offense."""
...
class Clamp(AlienPower):
    """Clamp - Holding Tool. +4 on defense."""
...
class Vise(AlienPower):
    """Vise - Clamping Tool. +5 on defense."""
...
class Mallet(AlienPower):
    """Mallet - Soft Hammer. +4 on offense."""
...
@dataclass
class Awl(AlienPower):
    """Awl - Piercing Tool. +4 on offense."""
    name: str = field(default="Awl", init=False)
    description: str = field(default="+4 when attacking.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.GREEN, init=False)
...
@dataclass
class Lathe(AlienPower):
    """Lathe - Turning Machine. +5 always."""
    name: str = field(default="Lathe", init=False)
...
# Register all powers
TOOL_EXTENDED_POWERS = [
    Drill, Saw, Pliers, Wrench, Screwdriver, Level, Tape_Measure, Chisel,
    File_Tool, Clamp, Vise, Mallet, Awl, Plane, Lathe,
]

for power_class in TOOL_EXTENDED_POWERS:
    try:
