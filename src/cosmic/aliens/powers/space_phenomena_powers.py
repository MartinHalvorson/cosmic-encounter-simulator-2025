"""
Space Phenomena Powers - Cosmic events themed aliens.
"""
...
@dataclass
class Black_Hole(AlienPower):
    """Black_Hole - Gravity Well. +5 on defense."""
    name: str = field(default="Black_Hole", init=False)
...
class Pulsar(AlienPower):
    """Pulsar - Rotating Star. Random +2 to +6."""
...
class Quasar(AlienPower):
    """Quasar - Bright Core. +5 on offense."""
...
class Neutron_Star(AlienPower):
    """Neutron_Star - Dense Core. +4 on defense."""
...
class White_Dwarf(AlienPower):
    """White_Dwarf - Dying Star. +3 always."""
...
class Red_Giant(AlienPower):
    """Red_Giant - Expanding Star. +5 on offense."""
...
class Dark_Matter(AlienPower):
    """Dark_Matter - Invisible Mass. +4 always."""
...
class Dark_Energy(AlienPower):
    """Dark_Energy - Expanding Force. +4 on offense."""
...
class Gamma_Burst(AlienPower):
    """Gamma_Burst - Energy Blast. +6 on offense."""
...
class Gravitational_Wave(AlienPower):
    """Gravitational_Wave - Space Ripple. Ships escape warp."""
...
class Magnetar(AlienPower):
    """Magnetar - Magnetic Star. -4 to opponent."""
...
class Cosmic_Ray(AlienPower):
    """Cosmic_Ray - Space Radiation. +3 on offense."""
...
class Event_Horizon(AlienPower):
    """Event_Horizon - Point of Return. Win ties automatically."""
...
@dataclass
class Solar_Flare_Alt(AlienPower):
    """Solar_Flare_Alt - Sun Burst. +5 on offense."""
    name: str = field(default="Solar_Flare_Alt", init=False)
    description: str = field(default="+5 when attacking.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    category: PowerCategory = field(default=PowerCategory.YELLOW, init=False)
...
class Singularity(AlienPower):
    """Singularity - Infinite Point. +5 always."""
...
# Register all powers
SPACE_PHENOMENA_POWERS = [
    Black_Hole, Pulsar, Quasar, Neutron_Star, White_Dwarf, Red_Giant,
    Dark_Matter, Dark_Energy, Gamma_Burst, Gravitational_Wave, Magnetar,
    Cosmic_Ray, Event_Horizon, Solar_Flare_Alt, Singularity,
]

for power_class in SPACE_PHENOMENA_POWERS:
    try:
