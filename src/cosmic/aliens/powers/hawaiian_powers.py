"""
Hawaiian Mythology Powers - Hawaiian folklore themed aliens.
"""
...
class Pele(AlienPower):
    """Pele - Volcano goddess. +5 on offense."""
...
class Kane(AlienPower):
    """Kane - Life god. +5 offense, -2 defense."""
...
@dataclass
class Milu(AlienPower):
    """Milu - Underworld god. Retrieve 3 ships."""
    name: str = field(default="Milu", init=False)
...
class Kanaloa(AlienPower):
    """Kanaloa - Sea god. See opponent card."""
...
class Papa(AlienPower):
    """Papa - Earth mother. +2 per home colony."""
...
@dataclass
class Maui_Hero(AlienPower):
    """Maui_Hero - Demigod. +3 first encounter."""
    name: str = field(default="Maui_Hero", init=False)
...
@dataclass
class Menehune(AlienPower):
    """Menehune - Little people. Random +0 to +6."""
    name: str = field(default="Menehune", init=False)
...
@dataclass
class Aumakua(AlienPower):
    """Aumakua - Family spirit. +3 defending home."""
    name: str = field(default="Aumakua", init=False)
...
@dataclass
class Nightmarchers(AlienPower):
    """Nightmarchers - Ghost warriors. -2 to opponent."""
    name: str = field(default="Nightmarchers", init=False)
...
@dataclass
class Laka(AlienPower):
    """Laka - Hula goddess. +1 per ally."""
    name: str = field(default="Laka", init=False)
...
@dataclass
class Wakea(AlienPower):
    """Wakea - Sky father. +1 per card."""
    name: str = field(default="Wakea", init=False)
...
@dataclass
class NamakaOkahai(AlienPower):
    """NamakaOkahai - Sea goddess. Ships go home."""
    name: str = field(default="NamakaOkahai", init=False)
...
# Register all powers
HAWAIIAN_POWERS = [
    Pele, Ku, Kane, Lono, Milu, Hina, Kanaloa, Papa, Maui_Hero, Menehune,
    Aumakua, Nightmarchers, Laka, Wakea, NamakaOkahai,
]

for power_class in HAWAIIAN_POWERS:
    try:
