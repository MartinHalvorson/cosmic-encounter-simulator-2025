"""
Computing Powers - Computer and IT themed aliens.
"""
...
class Algorithm(AlienPower):
    """Algorithm - Logical Process. +4 always."""
...
@dataclass
class Binary(AlienPower):
    """Binary - Zero and One. Random +0 to +8."""
    name: str = field(default="Binary", init=False)
    description: str = field(default="Random +0 to +8.", init=False)
    timing: PowerTiming = field(default=PowerTiming.REVEAL, init=False)
    power_type: PowerType = field(default=PowerType.MANDATORY, init=False)
    ...
    def modify_total(self, game: "Game", player: "Player", total: int, side: Side) -> int:
        if player.power_active:
            return total + (random.randint(0, 1) * 8)
        return total


@dataclass
class Firewall(AlienPower):
    """Firewall - Network Defense. +5 on defense."""
...
class Encryption(AlienPower):
    """Encryption - Coded Data. +4 on defense."""
...
class Virus_Alt(AlienPower):
    """Virus_Alt - Malicious Code. -4 to opponent."""
...
@dataclass
class Patch(AlienPower):
    """Patch - Bug Fix. Return 2 ships from warp."""
    name: str = field(default="Patch", init=False)
...
@dataclass
class Backup(AlienPower):
    """Backup - Data Copy. Ships escape warp."""
    name: str = field(default="Backup", init=False)
...
class Debug(AlienPower):
    """Debug - Error Finding. See opponent's card."""
...
class Cache(AlienPower):
    """Cache - Quick Storage. Draw extra card."""
...
class Bandwidth(AlienPower):
    """Bandwidth - Data Capacity. +3 always."""
...
class Compiler(AlienPower):
    """Compiler - Code Translator. +4 on offense."""
...
@dataclass
class Runtime(AlienPower):
    """Runtime - Execution Time. +2 per game turn."""
...
@dataclass
class Protocol(AlienPower):
    """Protocol - Communication Rules. Win ties automatically."""
    name: str = field(default="Protocol", init=False)
...
class Recursion(AlienPower):
    """Recursion - Self Reference. +5 on offense."""
...
class Server(AlienPower):
    """Server - Central Computer. +5 always."""
...
# Register all powers
COMPUTING_POWERS = [
    Algorithm, Binary, Firewall, Encryption, Virus_Alt, Patch, Backup,
    Debug, Cache, Bandwidth, Compiler, Runtime, Protocol, Recursion, Server,
]

for power_class in COMPUTING_POWERS:
    try:
